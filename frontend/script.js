...
const translations = {
    id: {
        ...
        uploadSuccess: "✓ Barcode berhasil terdeteksi!",
        uploadError: "❌ Tidak dapat mendeteksi barcode. Pastikan gambar jelas dan fokus.",
        ...
        backendError: "❌ Maaf, terjadi kesalahan saat menghubungi server AI"
    },
    en: {
        ...
        uploadSuccess: "✓ Barcode detected successfully!",
        uploadError: "❌ Cannot detect barcode. Please ensure image is clear and focused.",
...
async function scanImageForBarcode(imageDataUrl) {
    try {
        // Create a temporary image element to scan
...
function playAudio() {
    ...
    currentSpeech.onend = () => {
        playAudioBtn.style.display = 'flex';
        pauseAudioBtn.style.display = 'none';
        const endText = currentLanguage === 'id' ? '✓ Audio selesai diputar' : '✓ Audio finished';
    ...
    currentSpeech.onerror = (event) => {
        ...
        playAudioBtn.style.display = 'flex';
        pauseAudioBtn.style.display = 'none';
        const errorText = currentLanguage === 'id' ? '❌ Gagal memutar audio' : '❌ Failed to play audio';
...
window.addEventListener('beforeunload', () => {
    // 🛑 Hentikan semua audio saat window ditutup
    if (currentAudio) {
        currentAudio.pause();
...
function openChat() {
    ...
    const welcomeMessage = currentLanguage === 'id' 
        ? `Halo! 👋 Saya Musi, maskot museum. Saya akan dengan senang hati menjawab pertanyaan Anda tentang ${getText(currentItemContext, 'title')}. Ada yang ingin Anda tanyakan?`
        : `Hello! 👋 I'm Musi, the museum mascot. I'll be happy to answer your questions about ${getText(currentItemContext, 'title')}. What would you like to know?`;
...
function closeChat() {
    ...
    // 🛑 Hentikan audio AI saat modal ditutup
    if (currentAudio) {
        currentAudio.pause();
        currentAudio.currentTime = 0;
        currentAudio = null;
    ...
    // Hentikan text-to-speech jika ada
    if (window.speechSynthesis.speaking) {
        window.speechSynthesis.cancel();
    }
}
...
async function sendMessage() {
    ...
    try {
        ...
        const errorMessage = translations[currentLanguage].backendError || 
            (currentLanguage === 'id'
                ? '❌ Maaf, terjadi kesalahan saat menghubungi server AI'
                : '❌ Sorry, an error occurred while contacting the AI server');
